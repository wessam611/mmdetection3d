...
from mmdet3d.datasets.transforms.waymo_utils import convert_range_image_to_point_cloud
...
def load_frame_inputs(frame) -> tuple:
    ...
    range_images = dict(
        [(k, range_images[k]) for k in [open_dataset.LaserName.TOP]]
    )
    camera_projections = dict(
        [(k, camera_projections[k]) for k in [open_dataset.LaserName.TOP]]
    )
    ...
    for laser_name in range_images.keys():
        ...
        for ri in [0, 1]:
            ...
            nlz_points[offset: offset+of_dif] = range_image_tensor.numpy()[range_image_mask.numpy(), -1]
...
def create_pd_file_example(out_path):
    """Creates a prediction objects file."""
    cloud_bucket = tfds.core.Path(
        f"gs://waymo_open_dataset_v_1_4_1/"
    )
    for split in ['training', 'validation', 'testing']:
        split_files = tf.io.gfile.glob(
            os.path.join(
                cloud_bucket, f'individual_files/{split}/segment*'
            )
        )
        def write_file(file):
            frame = open_dataset.Frame()
            ds = tf.data.TFRecordDataset([file], num_parallel_reads=1, compression_type="")
            with tf.io.TFRecordWriter(f'{out_path}/{split}/{file.split("/")[-1]}') as writer:
                for elem in tqdm(ds):
                    frame.ParseFromString(bytearray(elem.numpy()))

                    points, nlz_points, range_index = load_frame_inputs(frame)
                    feature = {
                        'frame': tf.train.Feature(bytes_list=tf.train.BytesList(value=[frame.SerializeToString()])),
                        'points': tf.train.Feature(bytes_list=tf.train.BytesList(value=[points.tobytes()])),
                        'nlz_points': tf.train.Feature(bytes_list=tf.train.BytesList(value=[nlz_points.tobytes()])),
                        'range_index': tf.train.Feature(bytes_list=tf.train.BytesList(value=[range_index.tobytes()]))
                    }

                    # Create an example and write it to the output TFRecord
                    example = tf.train.Example(features=tf.train.Features(feature=feature))
                    writer.write(example.SerializeToString())

        Parallel(n_jobs=1)(delayed(write_file)(file) for file in split_files)
    
create_pd_file_example('/home/source/mmdetection3d/data/waymo/waymo_format/records')