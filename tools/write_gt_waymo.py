...
gt_base = "/home/wessam/src/ransac_preprocessing/pointclouds/pkl_data_aligned/"
    ...
    with torch.no_grad():
        for i, (data_batch, paths) in tqdm.tqdm(enumerate(dataloader)):
            ...
            for i, path in enumerate(paths):
                points = data_batch['inputs']['points'][i].cpu().numpy()
                bboxes = data_batch['data_samples'][i].gt_instances_3d['bboxes_3d'].cpu().numpy()
                classes = data_batch['data_samples'][i].gt_instances_3d['labels_3d'].cpu().numpy()
                data_dict = {
                    'frame': None,
                    'points': points,
                    'nlz_points': None,
                ...
                pickle.dump(data_dict, open(os.path.join(gt_base, path), 'wb'))
                # raise ';lkjhg'
